...
(expensive, and it induces a maintenance load on the infrastructure side), or you ship the source and require users to install
the toolchain for your native code themselves - a toolchain that isn't bundled with the interpreter. <br>
Native extensions typically require at least a C compiler, and often CMake and/or other build tools.<br>
The consequence is that calling `pip install` on these projects will fail if the toolchain is not installed
on the system, breaking the (usual) simplicity of Python package management. <br><br>
Smelt leverages the Zig compiler, which is fully standalone (unlike most C toolchains) and is available as a PyPI package.
...
It also enables some interesting cross-compilation capabilities (for wheels and standalone binaries), although this is still experimental and comes with limitations.

### Unified, modern `pyproject.toml`-based interface over several binary backends

Besides hand-crafting native extensions in C, C++, Rust or Zig, there are existing tools in the Python ecosystem to convert Python code to native code.<br>
...
Another issue is that their "public" interface is usually built around a Python -> binary conversion, while internally they are actually Python -> transpiled C -> binary. Smelt provides the following features:

* Supports Cython, mypyc, Nuitka, and vanilla C/Zig native extensions under one unified `pyproject.toml`-based API.
* Isolates the C transpilation from the actual compilation in all of these, and injects Smelt's own Zig-based toolchain, to get truly standalone building (and enable *some* cross-compilation support).
* Provides a CLI frontend for developers to easily manipulate them (e.g. toggle native code on and off).
...
Smelt uses [Nuitka](https://nuitka.net/) to build standalone binaries from Python entrypoints.
...
* Plugging the pipeline for the other supported binary backends (Cython/mypyc) into Nuitka. Smelt will pre-build the native extensions, and hand the built files to Nuitka (some backends like mypyc generate additional runtime files that are not discovered by Nuitka automatically).
* Using the Zig compiler with Nuitka (**Note: Nuitka >=4.0.0 now supports the Zig compiler natively, but it was not the case before**) - and thus benefits from its standalone nature.
