...
| `entrypoints` | table of tables | Customizes the standalone-binary build (see [CLI](cli.md#build-standalone-binary)) and the bundler (see [Bundler: Reference](bundler-reference.md)) for one entrypoint. Keyed by the script name from `[project.scripts]`, or by an explicit `module.path`/`module.path:func`. Every `[project.scripts]` entry is picked up automatically with default options; declaring it here only lets you override those options (`include-modules`, `include-package`, `include-package-data`, `extra_flags`, plus every `build-dist` option listed in the bundler reference) or declare an additional entrypoint that isn't in `[project.scripts]`. |
